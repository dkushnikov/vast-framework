...
The V → A → S → T cascade is only valid if each layer hands the next what that layer needs in order to commit. These handoffs are **invariants**: the *content* varies by instance and scope, but the *presence* of each payload must hold — otherwise the downstream layer is committing against a gap. This is the concrete content behind the [Kernel](kernel.md) rule *"Architecture is made explicit before Strategy commits,"* generalized to every boundary.
...
**Invariant:** a committed, falsifiable Vision exists *before* Architecture commits the framework. (Absent or mis-shaped → AP-05 Vision-as-Use-Case, AP-07 missing triggers: Architecture ends up codifying a feature list or an un-revisable wish.)
...
**Invariant:** a prioritized set of selected compositions, with their validation intent, exists *before* Tactics delivers. (Absent → Tactics ships without direction: motion without traction.)

## Why these are invariants, not templates
